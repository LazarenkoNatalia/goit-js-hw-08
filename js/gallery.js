...
function handleGalleryClick(event) {
  ...
  const galleryOriginal = currentGallery.dataset.source;
  const galleryAlt = currentGallery.alt;
    
  
  const instance = basicLightbox.create(`
    <div class="modal">
      <img src="${galleryOriginal}" alt="${galleryAlt}" width=1112 heigth=640 >
    </div>`,
    {
  ...
  // { window.addEventListener("keydown", handleModalClick) },
      // onClose: (instance) => { window.removeEventListener("keydown", handleModalClick) }
    }
  );
